...
1) echo "Crear escenario"
   ...
   lxc launch ubuntu:j haproxy -c user.user-data="$(cat config_haproxy_lxd.yml)" -c user.network-config="$(cat network_haproxy.yaml)"  -p WAN-DMZ
   ...
   do
      ...
      sed -i "s/NOMBRE/haproxy-php-0$j/g" config_server_lxd_0$j.yml
      lxc launch ubuntu:j haproxy-php-0$j -c user.user-data="$(cat config_server_lxd_0$j.yml)" -c user.network-config="$(cat network_server_0$j.yaml)" -p DMZ
...
4) echo "Borrar escenario"
   lxc delete -f haproxy
   for j in {1..5}
   do
      lxc delete -f haproxy-php-0$j
   done
   echo " "
   lxc ls -c n,s,4,P,S,l,m
   ;;
