...
1) echo "Crear escenario"
   ...
   incus profile create WAN-LAN-DMZ
   cat profile_WAN-LAN-DMZ | incus profile edit WAN-LAN-DMZ
   ...
   incus launch images:ubuntu/22.04/cloud fw -c user.user-data="$(cat config_fw_incus.yml)" -c user.network-config="$(cat network_fw.yaml)" -p WAN-LAN-DMZ
   echo "------------------------------"
   echo "        creando server        "
   echo "------------------------------"
   ...
   incus launch images:ubuntu/22.04/cloud server -c user.user-data="$(cat config_server_incus.yml)" -c user.network-config="$(cat network_server.yaml)" -p DMZ
   echo "------------------------------"
   ...
   incus launch images:ubuntu/22.04/cloud admin2 -c user.user-data="$(cat config_fw_incus.yml)" -c user.network-config="$(cat network_admin2.yaml)" -p LAN
...
2) echo "Parar contenedores escenario"
   incus stop -f fw server admin2
   echo " "
   incus ls -c n,s,4,P,S,l
   ;;
3) echo "Arrancar containers escenario"
   incus start fw server admin2
   sleep 3
   echo " "
